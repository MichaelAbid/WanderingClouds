...
namespace WanderingCloud.Feature.Course
{
    [Serializable]
    ...
    public class CheckPoint : MonoBehaviour
    {
        ...
        {
            get { return previousCheckpoint == null; }
        }

        public bool isEndingPoint
        {
            get { return nextCheckpoint == null; }
        }
        ...
        {
            CheckPoint tempnextCheckpoint = GameObject.Instantiate(gameObject, transform.position, transform.rotation)
        ...
        private void OnDrawGizmos()
        {
            ...
            if (pawnTimers.Count > 0)
            {
                ...
                foreach (PawnTimer item in pawnTimers)
                {
                    Handles.Label(transform.position + (Vector3.up * 2) + (Vector3.up * i),
                        $"{item.pawn.name} : {item.timer}");
                    i++;
                }
            }
        }
